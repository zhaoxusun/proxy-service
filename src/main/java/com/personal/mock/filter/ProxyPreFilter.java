...
public class ProxyPreFilter extends ZuulFilter {
    ...
    private static Logger logger = LoggerFactory.getLogger(ProxyPreFilter.class);

    @Value(value = "${zuul.routes.proxy.path}")
    private String path;
    ...
    public Object run() {
        ...
        String url = mockApp.getProxyUrlPerfix();

        ProxyZuulRoute mockZuulRoute = new ProxyZuulRoute(mockAppId.toString(),path,url);

        //proxy服务转发请求，如果再次转向mock服务，不删掉mock相关header；否则删除mock相关header
        if (!(url.equals(mockRequestAddress) || url.equals(mockResponseAddress) || url.equals(proxyAddress) || url.equals(redirectAddress))){
